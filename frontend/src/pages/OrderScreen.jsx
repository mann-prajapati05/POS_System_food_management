...
export default function OrderScreen({
  ...
  const canSendToKitchen =
    ["draft", "pending"].includes(order?.status) && hasKitchenItems;
  const canProceedPayment =
    items.length > 0 && (!hasKitchenItems || order?.status === "completed");

  return (
    <section
      className="flex gap-0 animate-fade-in"
      ...
      <div className="flex w-[60%] flex-col bg-linen-bg">
        ...
        <div className="border-b border-linen-border p-4">
          ...
          <div className="relative mt-3">
            <svg
              className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-linen-text-muted"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
          ...
          <div className="grid grid-cols-3 gap-3">
            {products.map((product) => (
              <ProductCard
                ...
                product={product}
                onAdd={onAddItem}
                disabled={!canAddOrIncrease || busy}
              />
            ))}
          </div>
          {products.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <svg
                className="h-12 w-12 text-linen-border"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1}
                stroke="currentColor"
              ...
              <p className="mt-1 text-[13px] text-linen-text-muted">
